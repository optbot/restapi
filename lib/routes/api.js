...
router.get('/', function(req, res) {
  ...
  var expiryBegin = new Date(expiry.getFullYear(), expiry.getUTCMonth(), expiry.getUTCDate());
  expiryBegin.setUTCHours(0, 0, 0, 0);

  // Set the end of the day provided by going ahead one day then back one millisecond.
  var expiryEnd = new Date(expiryBegin.getFullYear(), expiryBegin.getUTCMonth(), expiryBegin.getUTCDate() + 1);
  expiryEnd.setUTCHours(0, 0, 0, 0);
  expiryEnd.setUTCMilliseconds(expiryBegin.getUTCMilliseconds() - 1);

  MongoClient.connect(process.env.npm_package_config_db_options, function(err, db) {
